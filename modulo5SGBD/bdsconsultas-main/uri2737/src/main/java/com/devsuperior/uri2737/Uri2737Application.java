...
public class Uri2737Application implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		List<LawyerMinProjection> lista = repository.buscaNativa();
		List<LawyerMinDTO> resultado1 = lista.stream().map(x -> new LawyerMinDTO(x)).collect(Collectors.toList());
		
		System.out.println("\n*** RESULTADO SQL RAIZ (Natvive Query): ");
		for (LawyerMinDTO obj : resultado1) {
