...
public interface EmpregadoRepository extends JpaRepository<Empregado, Long> {
	...
	List<EmpregadoDeptProjection> buscaNativa();
	
	@Query("SELECT new com.devsuperior.uri2990.dto.EmpregadoDeptDTO(obj.cpf, obj.enome, obj.departamentos.dnome) "
			+ "FROM Empregado obj "
			...
			+ ")  "
			+ "ORDER BY obj.cpf")
	List<EmpregadoDeptDTO> buscaJPQL();
	
	@Query(nativeQuery = true, value = "SSELECT empregados.cpf, empregados.enome, departamentos.dnome, trabalha.*  "
			+ "FROM empregados "
			+ "INNER JOIN departamentos ON empregados.dnumero = departamentos.dnumero "
			+ "LEFT JOIN trabalha ON trabalha.cpf_emp = empregados.cpf "
			+ "WHERE trabalha.cpf_emp IS NULL "
			+ "ORDER BY empregados.cpf")
	List<EmpregadoDeptProjection> buscaNativa2();
